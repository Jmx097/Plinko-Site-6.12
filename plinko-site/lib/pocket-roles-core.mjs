...
export async function getMemberRoleAccessWithConfig({ userId, config, fetchImpl = fetch, now = Date.now() } = {}) {
  ...
  const assignments = await client.get(`member_role_assignments?user_id=eq.${encodeURIComponent(userId)}&select=role_key,expires_at&order=role_key.asc`);
  const roles = assignments
    .filter((assignment) => ROLE_KEYS.has(assignment.role_key) && (!assignment.expires_at || Date.parse(assignment.expires_at) > now))
