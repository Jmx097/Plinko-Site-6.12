...
test('commission and payout amounts are derived and selected payouts retain an immutable server snapshot', () => {
  const ledger = tableBody('commission_ledger');
  assert.match(
    ledger,
  ...
  assert.match(items, /commission_cents_snapshot integer not null check \(commission_cents_snapshot > 0\)/);
  assert.match(items, /commission_ledger_id bigint unique not null/);
  assert.match(compact, /new\.commission_cents_snapshot := ledger_commission_cents/);
  assert.match(compact, /payout item ownership and commission snapshot are immutable/);
  assert.match(compact, /financial basis and attribution are immutable after payout selection/);
});
...
test('payout lifecycle is finite, timestamped, and guarded by hardened security-definer triggers', () => {
  ...
  assert.match(batches, /status text not null default 'draft' check \(status in \('draft', 'approved', 'submitting', 'submitted', 'paid', 'void'\)\)/);
  for (const column of ['submission_idempotency_key text unique', 'submission_claimed_at timestamptz', 'external_transfer_id text unique', 'submitted_at timestamptz']) {
    assert.match(batches, new RegExp(column));
  }
  ...
  assert.match(compact, /create function public\.prevent_payout_item_ledger_mutation\(\) returns trigger language plpgsql security definer set search_path = pg_catalog/);
  assert.match(compact, /create function public\.reconcile_paid_payout_batch\(\) returns trigger language plpgsql security definer set search_path = pg_catalog/);
  ...
  for (const fn of [
    ...
    'prevent_payout_item_ledger_mutation',
    'reconcile_paid_payout_batch',
